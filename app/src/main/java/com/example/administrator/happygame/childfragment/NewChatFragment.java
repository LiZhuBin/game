...
/**
 * 作者：Administrator on 2017/11/14 0014 13:21
 * 邮箱：xjs250@163.com
 */
...
public class NewChatFragment extends BaseFragment {
  ...
  News news;
    ...
    public static NewChatFragment getInstance() {
        NewChatFragment sf = new NewChatFragment();
        return sf;
    }
    ...
    public void onStickyEvent(final News news) {

        this.news = news;
        string = StringUtil.httpArray(news.getNew_comment());
        for (int i = 0; i < string.length; i++) {
            ...
            if (i % 2 == 0) {
                ...
                for (int i1 = 0; i1 < userList.size(); i1++) {
                    LogUtil.e(news.getNew_comment());
                    forumItemList.add(new ActivityForumItem(userList.get(i1).getImage(), userList.get(i1).getName(), "第" + StringUtil.getAddOne(i1) + "楼", news.getNew_build_time(), stringList.get(i1)));
                }
                initRecyclerView();
